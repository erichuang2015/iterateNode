...
(function ($) {
    $.fn.dragging = function (options) {
        ...
        var initialize = function initialize() {
            ...
            costrainY = containerHeight - el.outerHeight() > 0 ? containerHeight - el.outerHeight() : 0;
            startX = 0;
            startY = 0;
            $left = 0;
            $top = 0;
        ...
        var desktop = function desktop() {
            ...
            settings.handler.on('mousedown', function (e) {
                ...
                dragging = true;
                boxleft = settings.getCSSposition ? parseInt(el.css('left')) : el.position().left; // get left position of box ( some differences between .css and .position. settings.getCSSposition allow to choose )
            ...
            $(document).on('mousemove', function (e) {
                if (dragging) {
                    ...
                    if (!settings.axis || settings.axis.toUpperCase() == 'X') el.css({ 'left': $left + 'px' });
                    if (!settings.axis || settings.axis.toUpperCase() == 'Y') el.css({ 'top': $top + 'px' });
                }
            });
        };
        ...
        var mobile = function mobile() {
            ...
            HandlerEl.addEventListener('touchstart', function (e) {
                ...
                boxleft = settings.getCSSposition ? parseInt(el.css('left')) : el.position().left; // get left position of box
            ...
            HandlerEl.addEventListener('touchmove', function (e) {
                ...
                if (!settings.axis || settings.axis.toUpperCase() == 'X') DOMel.style.left = $left + 'px';
                if (!settings.axis || settings.axis.toUpperCase() == 'Y') DOMel.style.top = $top + 'px';
                e.preventDefault();
            }, false);
        ...
        if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) mobile();else desktop();
